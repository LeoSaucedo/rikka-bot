...
async def on_message(message):
    ...
    if message.content.startswith(command("insultnfsw ", message)):
        ...
        msg = "{0.author.mention} calls {0.mentions[0].mention} a " + insultnsfwlist[randint(0, insultCount)].format(message)
        await message.channel.send(msg)
        await message.delete()
        
    if message.content.startswith(command("insult ", message)):
        # Says a random insult using an insult generator
        msg = "{0.author.mention} calls {0.mentions[0].mention} a" + insultlist[randint(0,hugcount)].format(message)
        await message.channel.send(msg)
        await message.delete()
